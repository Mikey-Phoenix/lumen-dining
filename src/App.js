...
const NavLinks = ({ toggleNavBar }) => {
    ...
    const fetchUserName = async () => {
      if (user) {
        ...
        if (userDoc.exists()) {
          ...
          setUserName(userDoc.data().personal?.fullName ?? "Profile");
        }
      }
    };
    fetchUserName();
  ...
  return (
    <>
      ...
      {user ? (
        <Link to="/profile" className="text-white bg-[#800020] py-2 px-3 md:px-10 rounded-full no-underline">
          {userName || "Profile"}
        </Link>
      ) : (
        <Link to="/signin" className="text-white bg-[#800020] py-2 px-3 md:px-10 rounded-full no-underline">
          Log In
        </Link>
      )}
    </>
      ...
      {!loading && (
        <BrowserRouter>
          <div className="min-h-screen bg-gray-100">
            ...
            {/* ── Navbar ───────────────────────────────────────────── */}
            {isOpen ? (
              <nav className="fixed top-0 left-[50%] translate-x-[-50%] h-[90vh] md:h-16 w-[95vw] md:w-[80vw] mt-4 px-4 bg-[#1F1F22] md:bg-black/60 rounded-3xl md:rounded-full z-50">
                <div className="flex flex-wrap justify-between mt-5">
                  <Link to="/" className="text-white font-bold md:text-2xl h-fit no-underline">Lumen Dining</Link>
              ...
              <nav className="fixed top-0 left-[50%] translate-x-[-50%] min-h-10 md:h-16 w-[95vw] md:w-[80vw] mt-4 px-4 bg-black/70 md:bg-black/60 rounded-3xl md:rounded-full z-50 flex flex-wrap justify-between items-center">
                <Link to="/" className="text-white font-bold md:text-2xl no-underline">Lumen Dining</Link>
